...
# 使用 pkg 打包 Node.js 后端，生成独立可执行文件
...
# 1. 使用 pkg 打包 server.js
echo "1️⃣ 打包 Node.js 后端..."
npx pkg server.js --targets node18-macos-arm64 --output NewsWidget-Server
...
#!/bin/bash
cd "$(dirname "$0")"
./NewsWidget-Server > /tmp/widget.log 2>&1 &
sleep 1
./NewsWidget
EOF
